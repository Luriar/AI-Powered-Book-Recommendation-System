...
**Top-5 추천 결과**
핵심 알고리즘:
python# 벡터 유사도 30% + 감정 가중치 + 상황 가중치 + 주제 가중치
total_score = (
    0.30 * 벡터_유사도 +
    감정_가중치 * 감정_벡터_유사도 +
    상황_가중치 * 상황_벡터_유사도 +
...
도서 관리: ISBN 일괄 등록, 분류, 검색, 가격/재고 관리
AI 요약: 배치 처리, 멀티스레드, Rate Limiting
추천 시스템: 자연어 분석, 벡터 임베딩, 하이브리드 점수 계산
수집/대출: 위시리스트, 도서 대출/반납, 이력 조회
UI/UX: BGM 선택, 로딩 스켈레톤, 반응형 디자인
...
5. 기술 선택 과정
LLM API 비교 분석
API비용 (토큰당)한국어 지원선택 이유GPT-4$0.03우수성능 최고, 비용 높음 Google PaLM 2무료 쿼터보통한국어 성능 제한적 DeepSeekGPT-4의 1/10우수비용 효율 + 성능 균형
최종 선택: DeepSeek
...
벡터 임베딩 모델
모델특징선택 이유OpenAI Embedding유료 API, 성능 우수비용 부담 e5-large-v2오픈소스, GPU 가속 가능무료 + 로컬 처리
...
AI 요약 생성 속도: 8.7배 개선 (611초 → 70초)
추천 응답 속도: 2.3배 개선 (38초 → 16.7초)
임베딩 계산 속도: 3.2배 개선 (117ms → 37ms)
...
사용자 피드백 학습: 클릭/건너뛰기 행동 수집 → 추천 정확도 향상
모바일 반응형 디자인: 현재 PC 중심 → 모바일 최적화
벡터 DB 도입: MySQL → Pinecone/Milvus (벡터 검색 최적화)

장기 확장

로컬 LLM 배포: API 의존도 감소, 비용 절감
다국어 지원: 영어, 일본어 등 확장
협업 필터링 추가: 사용자 간 유사도 기반 추천


9. 기술 문서 링크

원본 졸업논문 (중국어, 60페이지): graduation_thesis_CN.pdf
시스템 아키텍처 다이어그램: 논문 3장 참조
API 명세서: 논문 4.2절 참조
성능 테스트 결과: 논문 5장 참조
